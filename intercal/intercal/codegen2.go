...
func CodeGen2(statements []*Statement, llvmVersion int, w io.Writer) error {
	state := codeGenState2{statements: statements, llvmVersion: llvmVersion}
...
type codeGenState2 struct {
	statements  []*Statement
	llvmVersion int

	stmtInfo      []*codeGenStmt2
	spots         map[Var16]*codeGenVar2
	twospots      map[Var32]*codeGenVar2
	tails         map[Array16]*codeGenVar2
	hybrids       map[Array32]*codeGenVar2
...
type codeGenStmt2 struct {
	abstained     bool
	reinstated    bool
	debugLocation string
}

type codeGenVar2 struct {
	ident   string
	is16    bool
	varType string
	valType string
...
func (cgs *codeGenState2) collectStmtInfo() {
	for _ = range cgs.statements {
		cgs.stmtInfo = append(cgs.stmtInfo, &codeGenStmt2{})
	}
...
func (cgs *codeGenState2) collectVarInfo() {
	cgs.spots = make(map[Var16]*codeGenVar2)
	cgs.twospots = make(map[Var32]*codeGenVar2)
	cgs.tails = make(map[Array16]*codeGenVar2)
	cgs.hybrids = make(map[Array32]*codeGenVar2)
	for _, stmt := range cgs.statements {
		if stmt.Error != nil {
			continue
		}
		...
		case StatementLibrary:
			stmt.Operands.(LibraryFunction).CollectVarInfo2(cgs)
...
func (cgs *codeGenState2) collectStmtVarInfo(arg interface{}, stmtType StatementType) *codeGenVar2 {
	var varInfo *codeGenVar2
	switch e := arg.(type) {
	case Var16:
		info, ok := cgs.spots[e]
		if !ok {
...
func (cgs *codeGenState2) collectVarDimInfo(dim Dimensioning) {
...
func (cgs *codeGenState2) varInfo(vrbl Ignoredable) *codeGenVar2 {
	switch v := vrbl.(type) {
	case Var16:
		return cgs.spots[v]
	case Var32:
...
func (cgs *codeGenState2) genListing(w io.Writer) error {
...
func (cgs *codeGenState2) genErrorMessages(w io.Writer) error {
	// error messages
	errorMessagesSize := 0
	...
	// void @fatal_error_on_the_way_to(i32 %err_code, void()* %stmt_ptr) noreturn
	if _, err := fmt.Fprintf(w, "define void @fatal_error_on_the_way_to(i32 %%err_code, void()* %%stmt_ptr) {\n"); err != nil {
		return err
	}
	for i := range cgs.statements {
		if _, err := fmt.Fprintf(w, "    %%check_stmt%d = icmp eq void()* %%stmt_ptr,@stmt%d\n", i, i); err != nil {
			return err
		}
...
func (cgs *codeGenState2) genGlobals(w io.Writer) error {
	// next/resume stack
	if _, err := fmt.Fprintf(w, "@stack = global [79 x void()*] zeroinitializer\n@stackptr = global i32 0\n"); err != nil {
		return err
	}
...
func (cgs *codeGenState2) genDebugInfo(w io.Writer) error {
...
func (cgs *codeGenState2) ident(name string) string {
	cgs.labelCounter++
	return fmt.Sprintf("%%%s%d.%d", name, cgs.stmt.Index, cgs.labelCounter)
}

func (cgs *codeGenState2) label(name string) string {
	cgs.labelCounter++
	return fmt.Sprintf("%s%d.%d", name, cgs.stmt.Index, cgs.labelCounter)
}

func (cgs *codeGenState2) nextIndex() int {
...
func (cgs *codeGenState2) codeGenStmt(w io.Writer) error {
	...
	case StatementCalculate:
		...
		if cgs.stmt.Chance > 100 {
			if _, err := fmt.Fprintf(w, "    br label %%%s%s\n", loopLabel, cgs.debugLocation); err != nil {
				return err
			}
		} else {
	...
	case StatementResume:
		...
		if err774Ident != "" {
			...
			if _, err := fmt.Fprintf(w, "  %s:\n    call void @fatal_error_on_the_way_to(i32 774,void()* %s)%s\n", err774Label, nextStmtIdent, cgs.debugLocation); err != nil {
				return err
			}
		...
		if _, err := fmt.Fprintf(w, "    musttail call void %s()%s\n    ret void\n", nextStmtIdent, cgs.debugLocation); err != nil {
			return err
		}
	...
	case StatementWriteIntoBit:
		...
		input0Label := cgs.label("input_zero")
		input1Label := cgs.label("input_one")
		if _, err := fmt.Fprintf(w, "    br i1 %s,label %%%s,label %%%s%s\n", inputbitIdent, input1Label, input0Label, cgs.debugLocation); err != nil {
			return err
		}
		if _, err := fmt.Fprintf(w, "  %s:\n    musttail call void @stmt%d()%s\n    ret void\n  %s:\n    musttail call void @stmt%d()%s\n    ret void\n", input1Label, cgs.stmt.Operands.([2]int)[1], cgs.debugLocation, input0Label, cgs.stmt.Operands.([2]int)[0], cgs.debugLocation); err != nil {
			return err
		}
		if _, err := fmt.Fprintf(w, "  %s:\n", gotEofLabel); err != nil {
			return err
	...
	case StatementLibrary:
		...
		cgs.stmt.Operands.(LibraryFunction).CodeGen2(w, cgs)
		if _, err := fmt.Fprintf(w, "    br label %%%s%s\n  %s:\n", loopEndLabel, cgs.debugLocation, loopEndLabel); err != nil {
			return err
		}
		if cgs.stmt.Chance > 100 {
...
func (cgs *codeGenState2) genCheckIgnored(w io.Writer, ignoredable Ignoredable) (string, error) {
...
func (cgs *codeGenState2) genExpr(w io.Writer, expr Expr) (string, error) {
...
func (cgs *codeGenState2) genGets(w io.Writer, lhs LValue, rhsIdent string, doneLabel string) error {
...
func (cgs *codeGenState2) genAccessVar(w io.Writer, resultIdent string, varInfo *codeGenVar2) error {
	if !varInfo.assigned {
		tag := 0
		if !varInfo.is16 {
			tag = 1
...
func (cgs *codeGenState2) genAccessArrayElt(w io.Writer, resultIdent string, arrayElement ArrayElement) error {
...
func (cgs *codeGenState2) genFindArrayElement(w io.Writer, boundsErrorLabel, indexErrorLabel string, arrayElement ArrayElement) ([][2]string, string, error) {
...
func (cgs *codeGenState2) genMemset(w io.Writer, memPtrIdent, byteValIdent, sizeIdent string) error {
...
func (cgs *codeGenState2) genPushNextStack(w io.Writer, indexToPush, indexForError int) error {
	stackptrIdent := cgs.ident("stackptr")
	if _, err := fmt.Fprintf(w, "    %s = load i32,i32* @stackptr%s\n", stackptrIdent, cgs.debugLocation); err != nil {
		return err
	}
	...
	if _, err := fmt.Fprintf(w, "    store void()* @stmt%d, void()** %s%s\n", indexToPush, stackentryptrIdent, cgs.debugLocation); err != nil {
		return err
	}
	return nil
}

func (cgs *codeGenState2) genGetRand1000(w io.Writer) (string, error) {
...
func (cgs *codeGenState2) genGotoNext(w io.Writer, nextIndex int, countIdent string) error {
	gotoChance := uint16(0)
	...
	for _, index := range cgs.stmt.Goto {
		...
		if cgs.stmt.Type != StatementNext {
			if _, err := fmt.Fprintf(w, "    musttail call void @stmt%d()%s\n    ret void\n", index, cgs.debugLocation); err != nil {
				return err
			}
		} else {
