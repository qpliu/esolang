...
func TestPushback(t *testing.T) {
	tokenizer := NewTokenizer("()", bytes.NewBufferString("(123)"))
	token, err := tokenizer.Next()
	if err != nil {
		t.Errorf("Unexpected error=%s", err.Error())
		return
	}
	if token.Type != TokenWax {
		t.Errorf("Unexpected token=%s", token.StringValue)
		return
	}
	token, err = tokenizer.Next()
	if err != nil {
	...
	if token.Type != TokenNumber || token.NumberValue != 123 {
		t.Errorf("Unexpected token=%s", token.StringValue)
		return
	}
