...
impl<'a> Ast {
    ...
    pub fn lookup_index(&'a self, name: &Symbol) -> Option<DefIndex> {
        match self.names.find(name) {
            None => None,
    ...
    pub fn lookup_index_by_str(&'a self, name: &str) -> Option<DefIndex> {
...
impl Expr {
        ...
        match Expr::parse_expr(tokens, start_index, bindings, names) {
            ...
            Ok((next_index,expr)) => {
                if next_index >= tokens.len() {
                    ...
                    match Expr::parse(tokens, next_index, bindings, names) {
                        Err(error) => Err(error),
    ...
    fn parse_expr(tokens: &[Token], start_index: uint, bindings: &HashMap<Symbol,uint>, names: &HashMap<Symbol,(uint,uint)>) -> Result<(uint,Expr),Error> {
        ...
        if tok.is_zero() || tok.is_one() || tok.is_nil() {
            return Ok(Expr::parse_literal(tokens, start_index));
        }
        assert!(tok.is_ident());
        ...
        for i in range(0,arity) {
            ...
            match Expr::parse_expr(tokens, expr_index, bindings, names) {
